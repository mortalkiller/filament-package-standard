...
- [ ] Strict test-runner behavior is enabled; warnings/risky/empty suites cannot silently pass.
...
Create a new immutable `vX.Y.Z` tag on the verified commit in `X.x`, then publish the GitHub Release. PATCH is a compatible fix, MINOR is compatible functionality, and MAJOR requires an incompatible consumer-facing change review and migration guide. Do not move, delete and recreate, or reuse published version tags. Clearly mark RC/beta releases as prereleases.
...
Release documentation uses the exact tag, checks major ancestry and successful exact-commit push CI, then publishes the major channel through `docs-production`. Only the highest stable semantic version can update Latest; old-major patches, prereleases and stale reruns must not overwrite newer docs. Other version directories are preserved.
...
Record the release and verification on relevant issues. After the first stable release of a newer package major is complete, update the default branch. A tag or a green CI run alone does not prove successful live publication.
